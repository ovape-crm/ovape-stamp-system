...
const messages: Record<string, { title: string; description: string }> = {
  success: {
    title: "성인 인증이 완료되었습니다",
    description:
      "이 창을 닫아도 됩니다. 인증을 요청한 담당자에게 결과가 전달됩니다.",
  },
  rejected: {
    title: "성인 인증을 완료할 수 없습니다",
    description: "성인으로 확인되지 않았습니다. 담당자에게 문의해 주세요.",
  },
  expired: {
    title: "인증 링크가 만료되었습니다",
    description: "담당자에게 새로운 인증 링크를 요청해 주세요.",
  },
  invalid: {
    title: "사용할 수 없는 인증 링크입니다",
    description: "이미 사용했거나 취소된 링크일 수 있습니다.",
  },
  unavailable: {
    title: "현재 인증을 시작할 수 없습니다",
    description: "서비스 설정이 완료되지 않았습니다. 담당자에게 문의해 주세요.",
  },
  error: {
    title: "인증 처리 중 문제가 발생했습니다",
    description: "잠시 후 다시 시도하거나 담당자에게 문의해 주세요.",
  },
};
...
export default function AdultVerificationPage() {
  ...
  useEffect(() => {
    if (result) return;
    fetch(`/api/adult-verification/requests/${encodeURIComponent(token)}`, {
      cache: "no-store",
    })
      .then(async (response) => {
  ...
  const key = result
    ...
    : status === "completed"
      ...
      : status === "cancelled"
        ...
        : status;
  const message =
    key !== "pending" && key !== "loading"
      ? (messages[key] ?? messages.error)
      : null;
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-10">
      <section className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm sm:p-8">
        <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-600 text-2xl font-bold text-white">
          B
        </div>
        {status === "loading" && !result ? (
          ...
          <>
            ...
            <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-left text-xs leading-5 text-amber-800">
              <strong className="block font-semibold">
                PASS 앱 인증이 되지 않나요?
              </strong>
              본인확인 화면에서 SMS 인증을 선택해 주세요. SMS로 진행해도 성인
              인증은 동일하게 완료됩니다.
            </div>

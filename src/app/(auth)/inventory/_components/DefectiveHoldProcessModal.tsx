...
type Action = "supplier_return" | "scrap";
...
export default function DefectiveHoldProcessModal({ hold, onClose, onComplete }: { hold: DefectiveInventoryHold; onClose: () => void; onComplete: () => Promise<void> | void }) {
  ...
  const canSubmit = action === "scrap" || (Boolean(supplierId) && numericAmount > 0);
  ...
  const submit = async () => {
    ...
    try {
      ...
      await onComplete();
      toast.success(action === "supplier_return" ? "도매처 반품 정산을 기록했습니다." : "자체 폐기를 처리했습니다.");
      onClose();
    } catch (error) {

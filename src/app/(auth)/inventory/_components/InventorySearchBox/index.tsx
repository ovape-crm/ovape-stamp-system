...
import { InventorySearchCondition } from '@/app/_domains/_inventory/_queryKeys/inventoryKeys';
...
const InventorySearchBox = ({ categories, onSearch }: InventorySearchBoxProps) => {
  const [useStatus, setUseStatus] = useState('');
  const [categoryId, setCategoryId] = useState('');
  const [searchTarget, setSearchTarget] = useState('item_name');
  const [keyword, setKeyword] = useState('');
  const [conditions, setConditions] = useState<InventorySearchCondition[]>([]);
  ...
  const fireSearch = (
    ...
    nextConditions: InventorySearchCondition[],
  ) => {
    onSearch?.({

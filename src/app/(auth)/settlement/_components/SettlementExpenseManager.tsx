...
export default function SettlementExpenseManager() {
  ...
  const recurringTemplates = (expensesQuery.data ?? []).filter((expense) => expense.is_recurring);
  const occurredExpenses = (expensesQuery.data ?? []).filter((expense) => !expense.is_recurring);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3"><p className="text-xs text-gray-600 sm:text-sm"><span className="font-semibold text-brand-600">{occurredExpenses.length}</span>건</p><input type="month" value={month} onChange={(e) => setMonth(e.target.value)} className={`${fieldClass} w-[180px] cursor-pointer`} /></div>
      <section className="expense-management-table overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm [&_table_th:not(:last-child)]:border-r [&_table_th:not(:last-child)]:border-gray-200 [&_table_td:not(:last-child)]:border-r [&_table_td:not(:last-child)]:border-gray-100">
        {expensesQuery.isPending ? <Loading size="sm" text="비용을 불러오는 중..." /> : occurredExpenses.length ? <div className="overflow-x-auto"><table className="w-full min-w-[860px] table-fixed text-sm"><colgroup><col className="w-[112px]" /><col className="w-[96px]" /><col className="w-[72px]" /><col className="w-[76px]" /><col /><col className="w-[112px]" /><col className="w-[120px]" /></colgroup><thead className="bg-gray-50/70 text-left text-xs text-gray-600"><tr><th className="whitespace-nowrap px-4 py-3">발생일</th><th className="whitespace-nowrap px-4 py-3">항목</th><th className="whitespace-nowrap px-4 py-3">매장</th><th className="whitespace-nowrap px-4 py-3">구분</th><th className="px-4 py-3">메모</th><th className="whitespace-nowrap px-4 py-3 text-right">금액</th><th className="whitespace-nowrap px-4 py-3 text-center">관리</th></tr></thead><tbody className="divide-y divide-gray-100">{occurredExpenses.map((expense) => <tr key={expense.id}><td className="whitespace-nowrap px-4 py-3">{expense.expense_date}</td><td className="whitespace-nowrap px-4 py-3 font-semibold text-gray-900">{expense.category}</td><td className="whitespace-nowrap px-4 py-3">{storeLabels[expense.store]}</td><td className="whitespace-nowrap px-4 py-3">{expense.recurrence_source_id ? "반복 발생" : "일회성"}</td><td className="break-words whitespace-normal px-4 py-3 text-gray-500">{expense.note || "-"}</td><td className="whitespace-nowrap px-4 py-3 text-right font-semibold">{expense.amount.toLocaleString("ko-KR")}원</td><td className="whitespace-nowrap px-4 py-3 text-center"><div className="flex justify-center gap-2"><Button size="xs" variant="gray" onClick={() => startEditing(expense)}>수정</Button><Button size="xs" variant="gray" onClick={() => deleteMutation.mutate(expense.id)}>삭제</Button></div></td></tr>)}</tbody></table></div> : <p className="px-4 py-10 text-center text-sm text-gray-500">해당 월에 발생한 기타비용이 없습니다.</p>}
      </section>
      {recurringTemplates.length > 0 && <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm"><div className="border-b border-gray-200 bg-gray-50/70 px-4 py-3 text-sm font-bold text-gray-900">반복 지출 설정</div><div className="divide-y divide-gray-100">{recurringTemplates.map((expense) => { const day = expense.recurrence_day ?? Number(expense.expense_date.slice(8, 10)); return <div key={expense.id} className="flex items-center justify-between gap-3 px-4 py-3 text-sm"><div><span className="font-semibold text-gray-900">{expense.category}</span><span className="ml-2 text-gray-500">매월 {day}일 · {expense.amount.toLocaleString("ko-KR")}원</span></div><div className="flex gap-2"><Button size="xs" variant="gray" onClick={() => startEditing(expense)}>수정</Button>{!expense.recurrence_cancelled_on && <Button size="xs" variant="secondary" onClick={() => cancelMutation.mutate({ id: expense.id, cancelledOn: currentDate })}>반복 취소</Button>}</div></div>; })}</div></section>}
    </div>
  );
}

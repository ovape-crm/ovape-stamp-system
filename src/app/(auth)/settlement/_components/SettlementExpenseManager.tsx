  ...
  return (
    <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        {expensesQuery.isPending ? <Loading size="sm" text="비용을 불러오는 중..." /> : occurredExpenses.length ? <div className="overflow-x-auto"><table className="w-full min-w-[940px] text-sm"><thead className="bg-gray-50/70 text-left text-xs text-gray-600"><tr><th className="px-4 py-3">발생일</th><th className="px-4 py-3">항목</th><th className="px-4 py-3">매장</th><th className="px-4 py-3">구분</th><th className="px-4 py-3">메모</th><th className="px-4 py-3 text-right">금액</th><th className="px-4 py-3 text-center">관리</th></tr></thead><tbody className="divide-y divide-gray-100">{occurredExpenses.map((expense) => <tr key={expense.id}><td className="px-4 py-3">{expense.expense_date}</td><td className="px-4 py-3 font-semibold text-gray-900">{expense.category}</td><td className="px-4 py-3">{storeLabels[expense.store]}</td><td className="px-4 py-3">{expense.recurrence_source_id ? "반복 발생" : "일회성"}</td><td className="px-4 py-3 text-gray-500">{expense.note || "-"}</td><td className="px-4 py-3 text-right font-semibold">{expense.amount.toLocaleString("ko-KR")}원</td><td className="px-4 py-3 text-center"><div className="flex justify-center gap-2"><Button size="xs" variant="gray" onClick={() => startEditing(expense)}>수정</Button><Button size="xs" variant="gray" onClick={() => deleteMutation.mutate(expense.id)}>삭제</Button></div></td></tr>)}</tbody></table></div> : <p className="px-4 py-10 text-center text-sm text-gray-500">해당 월에 발생한 기타비용이 없습니다.</p>}
      </section>

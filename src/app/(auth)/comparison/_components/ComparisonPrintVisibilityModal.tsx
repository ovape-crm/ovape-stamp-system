...
export default function ComparisonPrintVisibilityModal({ onCancel }: { onCancel: () => void }) {
  ...
  return <div className="flex max-h-[calc(90vh-2rem)] min-h-0 w-full flex-col"><h2 className="mb-2 shrink-0 text-lg font-semibold">표 출력 여부</h2><p className="mb-4 shrink-0 text-sm text-gray-500">체크한 항목만 기기 비교표에 표시됩니다.</p>{isLoading ? <Loading size="sm" text="불러오는 중..." /> : <div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">{columns.map((column) => <label key={column.id} className="flex cursor-pointer items-center justify-between rounded-lg border border-gray-200 px-3 py-2.5 hover:bg-gray-50"><span className="text-sm text-gray-800">{column.name}</span><input type="checkbox" checked={checked(column.id, column.is_visible_in_comparison)} onChange={(event) => setVisible((prev) => ({ ...prev, [column.id]: event.target.checked }))} className="h-4 w-4 cursor-pointer accent-brand-500" /></label>)}</div>}<div className="mt-5 flex shrink-0 justify-end gap-2 border-t border-gray-200 pt-3"><Button size="sm" variant="gray" onClick={onCancel}>취소</Button><Button size="sm" disabled={saving || isLoading} onClick={save}>저장</Button></div></div>;
}

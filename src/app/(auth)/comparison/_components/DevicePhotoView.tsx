...
export default function DevicePhotoView({ deviceId, deviceName, onNavigate, onClose }: { deviceId: string; deviceName: string; onNavigate: (view: GuideView) => void; onClose: () => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[4100] flex items-center justify-center"><div className="absolute inset-0 bg-black/60" onClick={onClose} /><div className="relative z-10 flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-brand-100 px-6 py-4"><h2 className="text-lg font-semibold text-gray-900">{deviceName} 사진</h2><div className="flex items-center gap-4"><GuideNavigationDropdown current="photo" onNavigate={onNavigate} /><button type="button" onClick={onClose} className="cursor-pointer text-2xl leading-none text-gray-400 hover:text-gray-700" aria-label="닫기">×</button></div></div><div className="overflow-y-auto p-6">{photo?.header_content && <div className="mb-5 text-lg leading-relaxed text-gray-900"><TaggedContent content={photo.header_content} /></div>}{urls.length ? urls.map((url, index) => <ZoomableImage key={`${url}-${index}`} src={source(url)} alt={`${deviceName} 사진 ${index + 1}`} className={`mb-5 ${alignment}`} style={{ width: `${photo?.image_width_percent ?? 70}%` }} />) : !photo?.header_content && <p className="text-gray-500">등록된 기기 사진 또는 안내가 없습니다.</p>}</div></div></div>, document.body);
}

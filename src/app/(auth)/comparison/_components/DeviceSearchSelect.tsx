  ...
  return <div ref={containerRef} className="relative">
    ...
    <div className="relative">
      ...
      <input
        ...
        className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-9 pr-16 text-sm font-medium text-gray-900 shadow-sm outline-none transition placeholder:font-normal placeholder:text-gray-500 hover:border-brand-300 focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
      />
      {selected && !isOpen && <span className={`pointer-events-none absolute right-8 top-1/2 -translate-y-1/2 text-sm font-bold ${selected.hasSavedData ? 'text-emerald-600' : 'text-rose-500'}`} aria-label={selected.hasSavedData ? '저장된 데이터 있음' : '저장된 데이터 없음'}>{selected.hasSavedData ? 'O' : 'X'}</span>}
      ...
      {filteredOptions.length ? filteredOptions.map((option) => {
        ...
        return <button key={option.id} type="button" role="option" aria-selected={isSelected} onClick={() => select(option.id)} className={`flex w-full cursor-pointer items-center justify-between gap-3 px-3 py-2 text-left text-sm transition ${isSelected ? 'bg-gray-100 font-semibold text-gray-900' : 'text-gray-700 hover:bg-gray-50'}`}>
          ...
          <span className="ml-auto flex shrink-0 items-center gap-2">
            <span className={`text-sm font-bold ${option.hasSavedData ? 'text-emerald-600' : 'text-rose-500'}`} aria-label={option.hasSavedData ? '저장된 데이터 있음' : '저장된 데이터 없음'}>{option.hasSavedData ? 'O' : 'X'}</span>
            {isSelected && <svg className="h-4 w-4 text-brand-600" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 0 1 0 1.414l-8 8a1 1 0 0 1-1.414 0l-4-4a1 1 0 1 1 1.414-1.414L8 12.586l7.293-7.293a1 1 0 0 1 1.414 0Z" clipRule="evenodd" /></svg>}
          </span>
        </button>;

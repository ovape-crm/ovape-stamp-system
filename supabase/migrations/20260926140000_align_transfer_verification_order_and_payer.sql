-- 개별 이체내역은 매출 생성 시간과 결제 순서대로 검증한다.
-- 과거 특이사항의 입금자명은 더 이상 이체자명으로 사용하지 않는다.
...
begin
  ...
  with payment_rows as (
    select l.id::text as log_id, l.category, l.created_at, (p.ordinality - 1)::integer as payment_index,
    ...
    union all
    select l.id::text, l.category, l.created_at, 0, l.jsonb->>'paymentType', coalesce((l.jsonb->>'totalAmount')::numeric,0), l.jsonb, c.name
    from public.logs l left join public.customers c on c.id=l.customer_id
    ...
    select jsonb_build_object(
      ...
      'payerName', case
        ...
        when jsonb ? 'reservationCreatedAt' then concat(
          ...
          ' (', to_char((jsonb->>'reservationCreatedAt')::timestamptz at time zone 'Asia/Seoul', 'MM/DD'), ' 예약, ', coalesce(nullif(trim(jsonb->>'reservationCreatedWorkerName'),''), nullif(trim(jsonb->>'createdWorkerName'),''), '작업자 미확인'), ') (재확인 : ', coalesce(nullif(trim(jsonb->>'confirmedWorkerName'),''), '작업자 미확인'), ')'
        )
        else coalesce(nullif(trim(jsonb->>'transferPayerName'),''), nullif(trim(jsonb->>'xCustomerName'),''), nullif(trim(customer_name),''), '입금자명 미확인')
      end,
      'amount', abs(amount)
    ) as entry, created_at, payment_index
    from payment_rows where payment_type in ('transfer','transfer_cash_receipt','egu_transfer','egu_transfer_cash_receipt')
  )
  select coalesce(jsonb_agg(entry order by created_at, payment_index), '[]'::jsonb) into v_expected from normalized;
  ...
  ) order by entry_order), '[]'::jsonb)
  into v_submitted from jsonb_array_elements(coalesce(p_entries,'[]'::jsonb)) with ordinality submitted(entry, entry_order);

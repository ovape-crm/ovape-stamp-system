...
begin
  ...
  v_memo := v_worker || ', 입력시간 ' || trim(trailing '.' from trim(to_char(v_hours, 'FM999999990.99'))) || '시간, 시급 ' || to_char(p_hourly_rate, 'FM999,999,999,990') || '원, 총 근무 횟수 ' || v_count || '회, 식대 ' || to_char(v_count*p_meal_allowance, 'FM999,999,999,990') || '원' || case when v_other_memo<>'' then ', ' || v_other_memo else '' end || ' = 총 ' || to_char(v_amount, 'FM999,999,999,990') || '원';

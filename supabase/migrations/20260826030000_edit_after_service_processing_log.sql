create or replace function public.edit_after_service_status_processing_log(
...
begin
  ...
  select log.id into v_target_log_id
  ...
  where log.after_service_id = p_after_service_id
    ...
    and (p_log_id is null or log.id = p_log_id)
  order by log.created_at desc, log.id desc
  limit 1;
  if v_target_log_id is null then raise exception 'AFTER_SERVICE_LOG_NOT_FOUND'; end if;
...
revoke all on function public.edit_after_service_status_processing_log(
  bigint, bigint, text, date, text, boolean, integer
) from public, anon, authenticated;
grant execute on function public.edit_after_service_status_processing_log(
  bigint, bigint, text, date, text, boolean, integer
) to authenticated;

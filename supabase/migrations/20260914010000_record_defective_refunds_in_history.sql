-- 불량 회수 환불도 고객 출고 이력에 남긴다. 불량품의 후속 재고 처리는
-- defective_inventory_holds에서 계속 관리한다.
...
-- 기존 불량 환불도 빠짐없이 이력에 복원한다.
insert into public.logs(admin_id, customer_id, category, action, note, jsonb, created_at)
select refund.created_by, refund.customer_id, 'stamp', 'refund', coalesce(refund.reason, ''),
  jsonb_build_object('refundId', refund.id, 'sourceLogId', refund.source_log_id, 'totalAmount', -refund.refund_amount,
    'payments', coalesce((select jsonb_agg(jsonb_build_object('paymentType', payment.payment_type, 'amount', payment.amount)) from public.customer_refund_payments payment where payment.refund_id = refund.id), '[]'::jsonb),
    'paymentType', source.jsonb->>'paymentType', 'storeName', source.jsonb->>'storeName', 'recoveryType', refund.recovery_type),
  refund.created_at
from public.customer_refunds refund
join public.logs source on source.id = refund.source_log_id
where refund.recovery_type = 'defective'
  and refund.status = 'completed'
  and not exists (select 1 from public.logs history where history.jsonb->>'refundId' = refund.id::text);

...
declare v_layer public.inventory_cost_layers%rowtype; v_event_type text;
begin
  ...
  select event_type into v_event_type from public.inventory_cost_events where id=v_layer.source_event_id;
  if v_event_type <> 'purchase_in' then raise exception 'DIRECT_COST_EDIT_REQUIRES_SOURCE_FLOW'; end if;
  ...
  update public.inventory_cost_events outbound set total_cost=totals.total_cost from (select outbound_event_id,sum(quantity*unit_cost)::integer total_cost from public.inventory_cost_allocations group by outbound_event_id having count(*) filter(where unit_cost is null)=0) totals where outbound.id=totals.outbound_event_id and exists(select 1 from public.inventory_cost_allocations allocation where allocation.outbound_event_id=outbound.id and allocation.source_layer_id=p_layer_id);
end $$;
